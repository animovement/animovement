...
#'
#' In `animovement`, we have a 0-dependency policy, which makes our packages
...
#' all of animovement' functionalities without errors.
#'
#' @param package If `NULL` or `"animovement"` (the default), all suggested
#'   packages for all 'animovement' packages will be installed. If specific
#'   packages are specified, only suggested packages for those packages will be
#'   installed.
#'
#' @details To reduce the dependency load, 'animovement' packages by default will
...
#' packages a given 'animovement' package might need. For example,
#' `install_suggested("see")`. `show_suggested()` is a convenient helper to show
#' the current list of suggested packages for each 'animovement' package.
#'
...
#' # download all suggested packages
#' if (FALSE) {
#'   install_suggested("animovement")
#' }
#'
#' # listing all reverse dependencies of animovement packages
#' show_reverse_dependencies()
#'
#' # listing all soft/weak dependencies of animovement packages
#' show_suggested()
#'
#' @export
install_suggested <- function(package = "animovement") {
  suggested_packages <- .suggested_pkgs()

  if (is.null(package) || "animovement" %in% package) {
...
#' @export
show_suggested <- function(package = "animovement") {
  suggested_packages <- .suggested_pkgs()
...
#' @rdname install_suggested
#' @export
show_reverse_dependencies <- function(package = "animovement") {
...
.suggested_pkgs <- function() {
  insight::compact_list(list(
    aniframe     = .find_suggested("aniframe"),
    aniread      = .find_suggested("aniread"),
    aniprocess   = .find_suggested("aniprocess"),
    animetric    = .find_suggested("animetric"),
    anivis       = .find_suggested("anivis"),
    anicheck     = .find_suggested("anicheck"),
  ))
}
...
.revdep_pkgs <- function() {
  ...
  insight::compact_list(list(
    aniframe     = .find_reverse_imports(cran_packages, "aniframe"),
    aniread      = .find_reverse_imports(cran_packages, "aniread"),
    aniprocess   = .find_reverse_imports(cran_packages, "aniprocess"),
    animetric    = .find_reverse_imports(cran_packages, "animetric"),
    anivis       = .find_reverse_imports(cran_packages, "anivis"),
    anicheck     = .find_reverse_imports(cran_packages, "anicheck"),
  ))
}
