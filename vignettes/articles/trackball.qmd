...
The optic flow sensors return readings in an arbitrary format. So if we would wish to calibrate the spatial units to *e.g.* mm, then we would need to make a calibration. Let's say the ball was **5 cm in diameter** and we made a test trial where we rotated the ball **10 times** by hand. We could then read that data and feed it to `calibrate_trackball` like this, And then use that to set the spatial unit with `set_unit_space`:
